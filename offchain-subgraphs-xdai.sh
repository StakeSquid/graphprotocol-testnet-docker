...
http  -b post https://api.thegraph.com/subgraphs/name/graphprotocol/graph-network-goerli query="{subgraphDeployments(first:$subsnumber, orderBy:signalledTokens, orderDirection:desc, where:{signalledTokens_gt:0, deniedAt:0, network:"gnosis"}) {ipfsHash}}" | jq -r .data.subgraphDeployments[].ipfsHash | sed -z 's/\n/,/g;s/,$/\n/' | tee offchain-subgraphs &> /dev/null &
